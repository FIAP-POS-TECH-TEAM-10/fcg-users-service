...
RUN --mount=type=secret,id=GITHUB_TOKEN \
    ...
    dotnet restore Fiap.FCGames.Catalogo.Api/Fiap.FCGames.Users.Api.csproj    
...
RUN --mount=type=secret,id=GITHUB_TOKEN \
    export NUGET_AUTH_TOKEN=$(cat /run/secrets/GITHUB_TOKEN) && \
    dotnet publish Fiap.FCGames.Catalogo.Api/Fiap.FCGames.Users.Api.csproj -c Release -o /app/publish /p:UseAppHost=false    
